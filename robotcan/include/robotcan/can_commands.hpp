...
{
  std::uint8_t flags = 0;
  if (cmd.arm_left_open)    flags |= FLAG_ARM_LEFT_OPEN;
  if (cmd.arm_left_close)   flags |= FLAG_ARM_LEFT_CLOSE;
  if (cmd.arm_right_open)   flags |= FLAG_ARM_RIGHT_OPEN;
  if (cmd.arm_right_close)  flags |= FLAG_ARM_RIGHT_CLOSE;
  if (cmd.water_main_open)  flags |= FLAG_WATER_MAIN_OPEN;
  if (cmd.water_main_close) flags |= FLAG_WATER_MAIN_CLOSE;
  if (cmd.emergency_stop)   flags |= FLAG_EMERGENCY_STOP;
  return flags;
}
...
{
  return (static_cast<std::uint32_t>(data[0]) << 0) |
         (static_cast<std::uint32_t>(data[1]) << 8) |
         (static_cast<std::uint32_t>(data[2]) << 16) |
         (static_cast<std::uint32_t>(data[3]) << 24);
}
...
inline void set_duty_for_valve_group(SprayDutyGroups & duty_groups, std::uint8_t valve_number, std::uint8_t duty)
{
...
{
  switch (mode) {
    case OperationMode::Driving: return "Driving";
    case OperationMode::Work:    return "Work";
...
inline std::string direction_to_string(DriveDirection direction)
{
  switch (direction) {
    ...
    case DriveDirection::Reverse: return "Reverse";
...
{
  switch (speed) {
    case DriveSpeed::Stop:   return "Stop";
    case DriveSpeed::Low:    return "Low";
    case DriveSpeed::Middle: return "Middle";
    case DriveSpeed::High:   return "High";
    default:                 return "Unknown";
  }
}
