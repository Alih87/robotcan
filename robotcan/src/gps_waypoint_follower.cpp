...
private:
  ...
  void timer_callback()
  {
    ...
    if (!mission_started_) {
      ...
      log_event("mission_started_moving_to_waypoint_1", 0, 0.0, 0);
    }
    ...
    if (distance_m <= waypoint_radius_m_) {
      handle_waypoint_reached(distance_m);
      return;
    }
    ...
    RCLCPP_INFO_THROTTLE(
      ...
      *this->get_clock(),
      1000,
      "Moving to waypoint %zu/%zu: distance=%.2f m mode=%s direction=%s speed=%d steering=%d",
  ...
  void handle_waypoint_reached(double distance_m)
  {
    if (!stopping_at_waypoint_) {
      ...
      RCLCPP_INFO(
        this->get_logger(),
        "Waypoint %zu reached. Distance %.2f m. Sending STOP.",
        current_waypoint_index_ + 1,
        distance_m);
    ...
    log_event(
      ...
      current_waypoint_index_,
      distance_to_waypoint_m(waypoints_[current_waypoint_index_]),
      0);
