...
public:
  ...
  : Node("velocity_model_odom_node")
  {
    ...
    this->declare_parameter<double>("tau_accel", 2.5);
    this->declare_parameter<double>("tau_decel", 1.5);
    this->declare_parameter<double>("velocity_scale", 1.0);
    this->declare_parameter<double>("deadband", 0.0);

    // If true, use IMU angular velocity Z instead of cmd_vel angular.z
    ...
    tau_accel_ =
      this->get_parameter("tau_accel").as_double();

    tau_decel_ =
      this->get_parameter("tau_decel").as_double();
    ...
    use_imu_yaw_rate_ =
      this->get_parameter("use_imu_yaw_rate").as_bool();
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "Velocity model odometry node started.");

    RCLCPP_INFO(
      this->get_logger(),
      "Publishing odometry: %s -> %s",
      odom_frame_.c_str(),
      base_frame_.c_str());
...
private:
  ...
  double apply_deadband_and_scale(
    double v_cmd,
    double yaw)
  {
  ...
  void timer_callback()
  {
    ...
    // 1. Yaw update
    //
    // Prefer IMU yaw rate when available.
    // ==========================================================
    ...
    const double v_target =
      apply_deadband_and_scale(
        cmd_linear_x_,
        yaw_);

    // ==========================================================
    // 3. Select accel/decel time constant
    // ==========================================================
    ...
    // ==========================================================
    // 4. First-order velocity response
    //
    // v_eff(k) =
    //   v_eff(k-1)
    //   + alpha * (v_target - v_eff(k-1))
    //
    // alpha = 1 - exp(-dt / tau)
    // ==========================================================
    ...
    // ==========================================================
    // 6. Create quaternion ONCE
    //
    // Both Odometry and TF use this exact quaternion.
    // ==========================================================

    const geometry_msgs::msg::Quaternion orientation =
      yaw_to_quaternion(yaw_);
    ...
    odom_msg.pose.covariance[21] =
      99999.0;   // roll

    odom_msg.pose.covariance[28] =
      99999.0;   // pitch
    ...
    // ==========================================================
    // 11. Publish odom -> base_link TF
    //
    // IMPORTANT:
    //
    // This transform contains EXACTLY the same pose as:
    //
    // odom_msg.pose.pose
    //
    // Parent:
    ...
    //
    // Rotation:
    //   same quaternion as odometry
    //
    // Timestamp:
    //   same timestamp as odometry
    // ==========================================================
